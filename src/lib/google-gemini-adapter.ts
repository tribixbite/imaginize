...
export class GoogleGeminiAdapter implements IAiClient {
  ...
  private convertFromGoogleFormat(
    ...
    if (!candidate.content || !candidate.content.parts) {
      throw new Error(
        `Google Gemini response has no content. Finish reason: ${candidate.finishReason || 'unknown'}`
      );
    }
  ...
  async create(
    ...
    if (params.stream) {
      ...
      throw new Error(
        'Streaming is not yet implemented for the Google Gemini native adapter.'
      );
    }
...
export function isGoogleNativeEndpoint(baseUrl: string): boolean {
  return (
    baseUrl.includes('generativelanguage.googleapis.com') && !baseUrl.includes('/openai')
  );
}
