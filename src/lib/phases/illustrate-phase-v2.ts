...
Return ONLY the style guide text, no JSON or formatting.`;
  ...
  private async processChapter(chapterNum: number): Promise<void> {
    ...
    if (concepts.length === 0) {
      await progressTracker.log(
        `No concepts found for chapter ${chapterNum} - skipping`,
        'warning'
      );
    ...
    const imageModelConfig = resolveModelConfig(config.imageEndpoint?.model || 'dall-e-3', config);
    const imageModel = typeof imageModelConfig === 'string' ? imageModelConfig : imageModelConfig.name;
  ...
  private async loadConceptsForChapter(chapterNum: number, chapterTitle: string): Promise<ImageConcept[]> {
    ...
    for (const line of lines) {
      ...
      if (line.startsWith('## ')) {
        ...
        // Inside target chapter - look for description
  ...
  private async generateImage(
    ...
    // Check if using Gemini image generation
    const isGemini = model.includes('gemini') || model.includes('image');
